...
def init_dashboard(flask_app):
    ...
    def update_graphs(gender, residence, age_range, smoking_status):
        ...
        fig1 = px.histogram(
            filtered_df, x='age', color='stroke',
            title='Stroke by Age', nbins=30,
            color_discrete_map={0: '#1f77b4', 1: '#d62728'}
        )
        fig1.update_layout(template='plotly_dark', plot_bgcolor='#111111')

        fig2 = px.histogram(
            filtered_df, x='smoking_status', color='stroke',
            title='Smoking Status vs Stroke',
            barmode='group',
            color_discrete_map={0: '#00cc96', 1: '#ff6692'}
        )
        fig2.update_layout(template='plotly_dark', plot_bgcolor='#111111')

        fig3 = px.scatter(
            filtered_df, x='bmi', y='avg_glucose_level', color='stroke',
            title='BMI vs Glucose Level',
            color_discrete_map={0: '#636efa', 1: '#ef553b'}
        )
        fig3.update_layout(template='plotly_dark', plot_bgcolor='#111111')

        stroke_only = filtered_df[filtered_df['stroke'] == 1]
        fig4 = px.histogram(
            stroke_only, x='age',
            title='Age Distribution of Stroke Cases',
            nbins=30,
            color_discrete_sequence=['#AB63FA']
        )
        fig4.update_layout(template='plotly_dark', plot_bgcolor='#111111')
